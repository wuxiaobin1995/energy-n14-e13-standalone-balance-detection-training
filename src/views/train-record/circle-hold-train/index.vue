...
-->
<template>
  <div class="circle-hold-train-record">
    <!-- 表格 -->
    <el-table
      class="table"
      :data="showData"
...
export default {
  ...
  methods: {
    ...
    handleToPdf(index, row) {
      this.$router.push({
        ...
        query: {
          ...
          routerName: JSON.stringify('/layout/circle-hold-train-record')
        }
      })
    },
    ...
    handleSecularTrend() {
      this.$router.push({
        path: '/layout/circle-hold-train-secular-trend-pdf',
        query: {
          routerName: JSON.stringify('/layout/circle-hold-train-record')
        }
      })
