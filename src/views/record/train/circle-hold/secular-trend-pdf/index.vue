<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-27 21:33:10
 * @LastEditTime: 2022-03-05 19:15:29
 * @Description : 圆圈保持训练-长期趋势报告
-->
<template>
  <div
    class="circle-hold-train-secular-trend-pdf"
    v-loading.fullscreen.lock="fullscreenLoading"
  >
    <!-- PDF区域 -->
    <div id="pdf" class="pdf-wrapper">
      <div class="up">
        <div class="title">圆圈保持训练-长期趋势报告</div>
        <div class="logo-img">
...
export default {
  ...
  methods: {
          ...
          for (let i = 0; i < res.length; i++) {
            ...
            for (let i = 0; i < element.trackArray.length; i++) {
              ...
              const r = parseFloat(
                Math.sqrt(
                  Math.abs(
                    Math.pow(x - element.xAxis, 2) +
                      Math.pow(y - element.yAxis, 2)
                  )
                ).toFixed(2)
              )
              if (r <= element.circle) {
    ...
    handlePrint() {
      this.$htmlToPdf(
        'pdf',
        `圆圈保持训练-长期趋势报告 ${this.$moment().format(
          'YYYY-MM-DD HH_mm_ss'
        )}`,
        580
      )
...
.circle-hold-train-secular-trend-pdf {
  width: 100%;
  height: 100%;
