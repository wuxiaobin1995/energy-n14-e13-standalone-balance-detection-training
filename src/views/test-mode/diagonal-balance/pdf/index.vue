...
<template>
  <div
    class="test-diagonal-balance-pdf"
  ...
  >
    ...
    <div id="pdf" class="pdf-wrapper">
      ...
      <div class="top">
        <div class="title">
          对角线平衡测试报告({{ pdfData.hospital ? pdfData.hospital : '' }})
        </div>
        <div class="logo">
...
export default {
  ...
  methods: {
    ...
    calculate(trackArray) {
      return new Promise((resolve, reject) => {
        ...
        if (front.length >= back.length) {
          this.deflection = this.deflection + '前'
        } else if (front.length < back.length) {
          this.deflection = this.deflection + '后'
        }
    ...
    handlePrint() {
      this.$htmlToPdf(
        'pdf',
        `对角线平衡测试报告 ${this.$moment().format('YYYY-MM-DD HH_mm_ss')}`,
        420
      )
    },
