 ...
 * @LastEditTime: 2022-04-21 16:56:31
 * @Description : 静态平衡测试-数据记录
-->
<template>
  <div class="static-balance-test-record">
    <!-- 表格 -->
    <el-table
      class="table"
      :data="showData"
...
export default {
  ...
  methods: {
    ...
    handleToPdf(index, row) {
      this.$router.push({
        ...
        query: {
          ...
          routerName: JSON.stringify('/layout/static-balance-test-record')
        }
      })
    },
    ...
    handleSecularTrend() {
      this.$router.push({
        path: '/layout/static-balance-test-secular-trend-pdf',
        query: {
          routerName: JSON.stringify('/layout/static-balance-test-record')
        }
      })
