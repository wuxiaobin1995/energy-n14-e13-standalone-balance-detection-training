<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-27 16:18:03
 * @LastEditTime: 2022-07-26 15:04:49
 * @Description : 圆圈保持训练-测量
-->
<template>
  <div class="circle-hold-train-measure">
    <!-- 标题 -->
...
export default {
  ...
  data() {
    return {
      /* 路由传参 */
      trainTime: JSON.parse(this.$route.query.trainTime),
      circle: JSON.parse(this.$route.query.circle),
      xAxis: JSON.parse(this.$route.query.xAxis),
      yAxis: JSON.parse(this.$route.query.yAxis),
      ...
      nowTime: JSON.parse(this.$route.query.trainTime), // 倒计时
      trackArray: [], // 轨迹数组
      pdfTime: '', // 报告保存时间
  ...
  methods: {
    ...
    handleCheckPdf() {
      this.$router.push({
        path: '/layout/circle-hold-train-pdf',
        query: {
          userId: JSON.stringify(this.$store.state.currentUser.userId),
          pdfTime: JSON.stringify(this.pdfTime),
          routerName: JSON.stringify('/layout/circle-hold-train-set')
        }
      })
    },
    ...
    handleGoBack() {
      this.$router.push({
        path: '/layout/circle-hold-train-set'
      })
    },
