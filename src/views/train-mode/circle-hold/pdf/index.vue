<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-27 16:18:52
 * @LastEditTime: 2022-07-26 15:04:59
 * @Description : 圆圈保持训练-PDF报告
-->
<template>
  <div
    class="train-circle-hold-pdf"
  ...
  >
    ...
    <div id="pdf" class="pdf-wrapper">
      ...
      <div class="top">
        <div class="title">
          圆圈保持训练报告({{ pdfData.hospital ? pdfData.hospital : '' }})
        </div>
        <div class="logo">
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        for (let i = 0; i < trackArray.length; i++) {
          ...
          const r = parseFloat(
            Math.sqrt(
              Math.abs(
                Math.pow(x - this.pdfData.xAxis, 2) +
                  Math.pow(y - this.pdfData.yAxis, 2)
              )
            ).toFixed(2)
          )
          if (r <= this.pdfData.circle) {
            yesArray.push(r)
          }
        }
        this.score = parseInt(
    ...
    handlePrint() {
      this.$htmlToPdf(
        'pdf',
        `圆圈保持训练报告 ${this.$moment().format('YYYY-MM-DD HH_mm_ss')}`,
        400
      )
    },
